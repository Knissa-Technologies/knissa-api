...
export class PaymentService {
  async execute(data: PaymentDTO) {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const currentWallet = await txWalletRepository.findById(wallet.id);
      ...
      // Debitar saldo
      await txWalletRepository.decreaseBalance(wallet.id, amount);
      ...
      const transaction = await txTransactionRepository.create({
        ...
        amount,
        netAmount: amount,
        description: data.description ?? `Payment to ${data.beneficiary}`,
        reference,
      });
    ...
    const updatedWallet = await walletRepository.findById(wallet.id);
