...
export class ExchangeService {
  async execute(data: ExchangeDTO) {
    ...
    const fromWallet = await walletRepository.findById(data.fromWalletId);
    ...
    const toWallet = await walletRepository.findById(data.toWalletId);
    ...
    if (fromWallet.userId !== toWallet.userId) {
      throw new AppError("Wallets must belong to the same user.", 400);
    }

    // Ambas devem estar ativas
    if (fromWallet.status !== WalletStatus.ACTIVE) {
      throw new AppError("Source wallet is not active.", 400);
    }

    if (toWallet.status !== WalletStatus.ACTIVE) {
      throw new AppError("Destination wallet is not active.", 400);
    }

    // As moedas precisam ser diferentes
    if (fromWallet.currencyId === toWallet.currencyId) {
      throw new AppError("Exchange requires different currencies.", 400);
    }

    // Buscar taxa de câmbio
    const exchangeRate = await exchangeRateRepository.findByCurrencies(
    ...
    const exchangeReference = generateTransactionReference();

    const debitReference = `${exchangeReference}-D`;

    const creditReference = `${exchangeReference}-C`;
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const txLedgerRepository = new LedgerRepository(tx);

      // Busca novamente a carteira dentro da transação
      const currentWallet = await txWalletRepository.findById(fromWallet.id);
      ...
      await txWalletRepository.decreaseBalance(fromWallet.id, amount);
      ...
      const debitTransaction = await txTransactionRepository.create({
      ...
      const creditTransaction = await txTransactionRepository.create({
